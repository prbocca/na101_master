...
######################################
##
## vertexattr_prediction.r
## 
######################################


nnm_node = function(g, attr="label", node, fun="freq"){
  result = NA
  if (fun=="mean"){
    result = mean(vertex_attr(g,attr, index=unlist(neighborhood(g, order=1, nodes=node, mindist=1))), na.rm=TRUE)
  } else if (fun=="freq"){
    tt = table(vertex_attr(g,attr, index=unlist(neighborhood(g, order=1, nodes=node, mindist=1))), useNA="no")
    result = as(names(tt[which.max(tt)]), class(vertex_attr(g,attr)))
  } 
  return(result)
}
# vertex_attr(g,attr, index=unlist(neighborhood(g, order=1, nodes=node, mindist=1)))
# [1] 4 4 4 4 6
# nnm_node(g,attr, 1, "mean")
# [1] 4.4
# > nnm_node(g,attr, 1, "freq")
# [1] 4

nnm = function(g, attr="label", fun="freq"){
  attr_result = unlist(sapply(V(g), function(x) nnm_node(g, attr, x, fun)))
  return(attr_result)
}


delete_vertex_attr_rand = function(g, attr="label", p=0.3){
  g_result = g
  vertex_attr_to_delete = ifelse(runif(vcount(g_result))<p,TRUE,FALSE)
  vertex_attr(g_result, attr)[vertex_attr_to_delete] = NA
  return(g_result)
}


evaluate_predictions_vertexattr = function(g, g_obs, attr, fun="mean", metric="acc"){
  
  require(ROCR)
  
  vertex_attr_deleted = is.na(vertex_attr(g_obs, attr)) #vector booleando con los atributos borrados
  vertex_attr_actual = vertex_attr(g, attr)[vertex_attr_deleted] #vector con los atributos reales
  vertex_attr_actual = factor(vertex_attr_actual, levels=unique(vertex_attr(g, attr)))
  
  #computo atributos predichos
  vertex_attr_pred  = nnm(g_obs,attr, fun)[vertex_attr_deleted] #vector con los atributos predichos
  #
  result=data.frame(true = vertex_attr_actual, pred=vertex_attr_pred)
  metrics = numeric()
  if (metric=="auc"){
    pred <- prediction(vertex_attr_pred, vertex_attr_actual)
    metrics = tryCatch({
      performance(pred, "auc")@y.values[[1]]
    }, error = function(e) {
      NaN
    ...
    cutoff_ind = tail(which(pred@cutoffs[[1]] >= 0.5), n=1)
    perf = performance(pred,'f')
    metrics = perf@y.values[[1]][cutoff_ind]
  } else if (metric=="acc"){
    
    if (length(unique(vertex_attr(g, attr)))==2){ #binario
      vertex_attr_pred = ifelse(vertex_attr_pred >= 0.5, 1,0)
    } else { #varias clases
      vertex_attr_pred = factor(vertex_attr_pred, levels=unique(vertex_attr(g, attr)))  
    }
    #
    # computo la precision
    cm = as.matrix(table(pred=vertex_attr_pred, actual=vertex_attr_actual)) # create the confusion matrix
    metrics = sum(diag(cm)) / sum(cm)
  }
  return(list(result=result, metrics=metrics))
}
...
summary_predictions_vertexattr = function(g,
                                          attr,
                                         n_sample=10, 
                                         p_v=seq(from=0.01,to=0.5,by=0.05), 
                                         fun="mean",
  ...
  if (cores==1){
    ...
    for (p in p_v){
      for (n in 1:n_sample){
        printf("%s %s %s",Sys.time(), p, n)
        g_obs = delete_vertex_attr_rand(g,attr, p=0.2)
        r = evaluate_predictions_vertexattr(g, g_obs, attr, fun, metric)
        r$metrics["p"] = p
        r$metrics["n"] = n
        results = rbind(results, r$metrics)
      }
    }
    names(results) = c(metric,"p","n")
    ...
    results = foreach(n = 1:n_sample, .combine=rbind) %dopar% {
      ...
      for (p in p_v){
        ...
        r$metrics["p"] = p
        r$metrics["n"] = n
        results_in = rbind(results_in, r$metrics)
      }
      names(results_in) = c(metric,"p","n")
      return(results_in)
    }
  }
  ...
  results_summary = aggregate(. ~ p, results[,setdiff(names(results),"n")], FUN="mean")
  results_summary_long = melt(results_summary, id.vars=c("p"))
  names(results_summary_long) = c("p","metric",metric)

  results_long = melt(results, id.vars=c("p","n"))
  names(results_long) = c("p","n","metric",metric)
